...
display(void)
{
	...
	int c = 8; //head node is 8 for CMan0023
  GLfloat m[16];
  ...
  glPopMatrix();
	...
	if ((z>0) && (fetch(iddata,avatar_info)>0))
	{
		...
		glLoadIdentity();
		glRotatef(avatar_info[7+(c*8)]*RADTODEG,avatar_info[4+(c*8)],avatar_info[5+(c*8)],avatar_info[6+(c*8)]);
...
main(int argc, char **argv)
{
  ...
  glutCreateWindow("EXT_texture_cube_map spherical display app");
